    ...
    function Controller(RestResourceService, LocationPointFactory, LocationPointService, ProjectFieldCenterService, $mdToast) {
        ...
        function onInit() {
            ...
            LocationPointService.getAll().then(function (response) {
                self.locationsPoints = LocationPointFactory.fromArray(response.data);
                _clearData();
            });
        }
        ...
        function removeLocation() {
            LocationPointService.removeLocationPoint(self.selectedLocationPoint.toJSON()).then(function (response) {
                if (response.data) {
                    self.locationsPoints.splice(self.selectedIndex, 1);
                    _clearData();
        ...
        function saveUser() {
            if (self.selectedUser){
                LocationPointService.saveUserLocation(self.selectedUser).then(function (response) {
        ...
        function removeUser(email) {
            LocationPointService.removeUserLocation(self.selectedLocationPoint._id, email).then(function (response) {
